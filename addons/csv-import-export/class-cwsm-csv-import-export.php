...
class CED_CWSM_CSV_Import_Export {
	...
	public function ced_cwsm_csv_import_export_hooks_and_filters_function() {
		add_action( 'admin_init', array( $this, 'ced_cwsm_csv_import_export_module_export_csv_format' ) );
		add_action( 'admin_init', array( $this, 'ced_cwsm_csv_import_export_module_download_error_log' ) );
	...
	public function ced_cwsm_csv_import_export_module_download_error_log() {
		...
		// Nonce Check
				...
				$get_nonce_from_url = isset( $_GET['nonce'] ) ? sanitize_text_field( $_GET['nonce'] ) : '';

		if ( empty( $get_nonce_from_url ) || ! wp_verify_nonce( $get_nonce_from_url, 'ced-csv_import-ajax-seurity-string' ) ) {
			die( 'Security check error' );
		}

		if ( strpos( sanitize_text_field( $_SERVER['REQUEST_URI'] ), 'ced_cwsm_log_download=' ) ) {
			$ced_cwsm_log_download = isset( $_GET['ced_cwsm_log_download'] ) ? sanitize_file_name( $_GET['ced_cwsm_log_download'] ) : '';
	...
	public function ced_cwsm_csv_import_export_module_export_csv_format() {
		...
		if ( strpos( $REQUEST_URI, '&tab=ced_cwsm_plugin&section=ced_cwsm_csv_import_export_module&ced_cwsm_export=true' ) ) {
			...
			$uploadDir   = $baseDir . '/wholesale_data_details';
			$nameTime    = time();
			if ( ! is_dir( $uploadDir ) ) {
				mkdir( $uploadDir, 0777, true );
			}
			$output = fopen( $uploadDir . '/wholesale_market_format.csv', 'w' );
			...
			fclose( $output );
			readfile( $uploadDir . '/wholesale_market_format.csv' );
			die();
		}
	}
		...
		if ( $check_ajax ) {
			...
			if ( ! is_dir( $importErrorDir ) ) {
				mkdir( $importErrorDir, $permissions = 0777 );
					...
					$logFileName    = 'csvImportError-' . time() . '.log';
					$errorLogReport = '';
						...
						while ( ( $data = fgetcsv( $handle ) ) !== false ) {
							...
							if ( 'true' == $response[0] ) {
								...
								if ( ! $anyIssueFound ) {
									...
									$errorLogReport .= 'Product-Name : ' . trim($data[2]) . "\n";
							...
							if ( 'true' == $response[1] ) {
								...
								$rsn = explode( '#', $response[1] ) ;
								$reason = end($rsn); 
								if ( ! $anyIssueFound ) {
									$anyIssueFound   = true;
									$errorLogReport .= __( 'Product-Name : ', 'wholesale-market' ) . $data[2] . "\n";
							...
							if ( 'true' == $woo_prc ) {
								...
								if ( 'true' == $response[0] ) {
									...
									if ( ! $anyIssueFound ) {
										$anyIssueFound   = true;
										$errorLogReport .= __( 'Product-Name : ', 'wholesale-market' ) . trim($data[2]) . "\n";
										$errorLogReport .= __( 'Update Failure Reason(s) :', 'wholesale-market' ) . '\n';
									}
									if ( 'empty' == $reason ) {
	...
	public function ced_cwsm_write_wholesale_csv_content_to_DB( $proId, $wholesale_price, $min_qty_to_buy ) {
		...
		$min_qty_to_buy = trim($min_qty_to_buy);
	...
	public function ced_cwsm_csv_import_export_module_admin_enqueue_scripts() {
		...
		if ( strpos( $req_uri, 'page=wholesale_market&tab=ced_cwsm_basic&section=ced_cwsm_csv_import_export_module' ) ) {
			wp_enqueue_script( 'ced_cwsm_csv_script_js', plugins_url( 'js/ced_cwsm_csv_script.js', __FILE__ ), array( 'jquery' ), time(), true );
			wp_localize_script(
