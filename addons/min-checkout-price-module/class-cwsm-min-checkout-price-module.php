...
class CED_CWSM_Min_Checkout_Price_Module {
	...
	public function ced_cwsm_min_pro_qty_module_add_hooks_and_filters() {
		...
		add_filter( 'woocommerce_get_sections_ced_cwsm_plugin', array( $this, 'ced_cwsm_min_checkout_price_module_add_section' ), 10, 1 );
	...
	public function woocommerce_before_cart_hidden_element() {
		...
		print_r( '<input type="hidden" name="isQualifyMinCheckoutPriceCondition" id="isQualifyMinCheckoutPriceCondition" value="' . $isQualifyMinCheckoutPriceCondition . '">' );
		print_r( '<div class="cwsm_custom_msg_error">' . $errorMsg . '</div>' );
		echo '</div>';
	}
